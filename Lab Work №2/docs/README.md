...
System(mta, "Клиент экскурсоводов", "Позволяет создавать маршруты и отправлять их туристам", $type="Container / iOS, Swift")
System(mt, "Клиент туристов", "Позволяет получать маршруты и просматривать их", $type="Container / iOS, Swift")
Container_Ext(link, "Сторонняя система создания ссылок", "System", "Сторонняя система позволящая создавать ссылки и qr-коды")
ContainerDb(db, "База данных", "Microsoft SQL", "Хранит маршруты и материалы связанные с маршрутами", $sprite="msql_server")
Boundary(s, "Сервер", "Container / C#, .NET") {
Container(cT, "Контроллер для туристов", "Component: Controller", "Обрабатывает запросы от мобильного клиента туриста. Принимает запросы на получение маршрутов и материалов для дальнейшего просмотра. Взаимодействует с Бизнес-логикой для предоставления данных туристу.")
Container(cTa, "Контроллер для экскурсоводов", "Component: Controller", "Обрабатывает запросы от мобильного клиента экскурсовода. Принимает запросы на создание и обновление маршрутов, загрузку материалов. Взаимодействует с Бизнес-логикой для обработки данных экскурсовода.")
Container(m, "Менеджер маршрутов и материалов", "Component: Controller", "Логика, связанная с обработкой данных от экскурсоводов и предоставлением их туристам. Получает данные от контроллеров для экскурсоводов и туристов. Взаимодействует с Базой данных для хранения и получения данных.")
}
BiRel(mt, cT, "http-запросы и ответы")
BiRel(mta, cTa, "http-запросы и ответы")
BiRel(m, db, "Сохранение и изъятие данных")
BiRel(m, link, "Создание и получение ссылок")
BiRel(m, cT, "")
BiRel(m, cTa, "")
SHOW_LEGEND()
@enduml
