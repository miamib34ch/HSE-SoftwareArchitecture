...
**Заголовки:**
- `Content-Type: multipart/form-data` для data
- `Content-Type: application/json` для остальных полей ответа
- `Authorization: Bearer {ваш_токен}`

**Входные параметры:**  
Query:  
- `material_id` (число) - Идентификатор материала, информацию о котором необходимо получить.
...
**Код статусы ответа:**
- `200 OK` - Успешный запрос, возвращается информация о материале
- `401 Unauthorized` - Ошибка авторизации, отсутствует или неверный токен
...
**cURL:** 
```
curl -X GET -H "Authorization: Bearer {ваш_токен}" https://api.example.com/material/9
```
...
**Входные параметры:**  
Query:  
- `material_id` (число) - Идентификатор материала, информацию о котором необходимо обновить.

Body:  
- `file` (файл, необязательно) - Новый файл.
...
**Код статусы ответа:**
- `200 OK` - Успешное обновление данных материала
- `401 Unauthorized` - Ошибка авторизации, отсутствует или неверный токен
- `404 Not Found` - Материал с указанным material_id не найден
- `400 Bad Request` - Некорректный запрос (например, неверные параметры в теле запроса)
...
{
  "material_id": 9,
  "title": "updated_material_photo.jpg"
}
```

**cURL:** 
```
curl -X PUT -H "Authorization: Bearer {ваш_токен}" -F "file=@updated_material_photo.jpg" https://api.example.com/material/9
```
